...
vi.mock('@tauri-apps/plugin-fs', () => ({
  readTextFile: vi.fn(), writeTextFile: vi.fn(), stat: vi.fn(),
}));
vi.mock('@tauri-apps/api/window', () => ({
  getCurrentWindow: () => ({
    setTitle: vi.fn(), onCloseRequested: vi.fn(), close: vi.fn(),
    destroy: vi.fn(), onDragDropEvent: vi.fn(),
  }),
}));
vi.mock('@tauri-apps/api/path', () => ({
  basename: vi.fn(), dirname: vi.fn(), resolve: vi.fn(), isAbsolute: vi.fn(),
}));
...
vi.mock('./spellcheck/custom-words.js', () => ({
  addCustomWord: vi.fn(), removeCustomWord: vi.fn(),
}));
vi.mock('./preview.js', () => ({
  render: vi.fn(), syncPreviewToCaret: vi.fn(), requestPreviewScrollToTop: vi.fn(),
}));
