...
//
// The panel selects its own search field when it mounts, but the Ex
// dialog is still closing: after this callback returns, the plugin's
// openDialog close path hides the dialog and synchronously refocuses the
// editor, which steals focus from the field. A microtask runs after that
// task, so refocusing there wins. The plugin's deferred blur-close fires
// later still but no-ops on an already-closed dialog, so nothing steals
// focus back a second time.
Vim.defineEx("find", "find", () => {
  if (!editorView) return;
  openSearchPanel(editorView);
  queueMicrotask(() => {
    const field = editorView?.dom.querySelector<HTMLInputElement>(".cm-search input[name=search]");
    field?.focus();
    field?.select();
  });
});
