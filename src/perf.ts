...
// narrowed to the literal `false`, which keeps `if (PERF)` below from
...
// build the message string unconditionally, since it is one short
// string per render pass, not a hot path, so there is no thunk
// indirection.
